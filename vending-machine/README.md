...
このディレクトリは、CQ出版社の雑誌『Interface』2024年6月号の第２部第２章の記事で解説した自動販売機シミュレーターに関するファイルを提供するものです。

## ファイル一覧

* `docs/prompt.txt` コード生成プロンプト
...
* `docs/prompt.txt`によって自動で生成したファイルを元に、不具合や不足機能を手動で修正して作ったプログラムです。
...
3. このディレクトリ（`vending-machine/`）に移動する
4. `dot.env` ファイルにAPIキーを記入して、ファイル名を `.env` にする
5. シェルで `npm i` と入力して、依存ライブラリをインストールする
6. `npm start` と入力して、サーバー側プログラムを起動する 
7. Webブラウザで http://localhost:3000 を開く
